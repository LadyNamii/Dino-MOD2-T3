...
class Game:
    ...
    def execute(self):
        ...
        while self.running:
            if not self.playing:
                self.show_menu() #chamando o menu # colocar a tela de instruçoes
        pygame.display.quit() # aqui ele pode voltar  pro jogo
        pygame.quit() # ou sair do jogo
    ...
    def update(self):
        user_input = pygame.key.get_pressed() # pra apertar qualquer tecla
    ...
    def draw_power_up_time(self): #pode acrescentar poderzinho
       if self.player.has_power_up:
           ...
           if time_to_show >= 0:
               draw_message_component(
                   f"{self.player.type.captalize()} FALTAM {time_to_show} SEGUNDOS",
                   self.screen,
                   font_size= 18,
                   pos_x_center= 500,
                   pos_y_center= 40
